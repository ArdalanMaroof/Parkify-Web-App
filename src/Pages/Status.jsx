...
const Status = () => {
  ...
  useEffect(() => {
      ...
      if (urlParams.get('success') === 'true') {
        ...
        if (pending) {
          ...
          try {
            ...
            await axios.post('http://localhost:2000/api/confirmed-parking', {
    ...
    const fetchActiveSession = async () => {
      try {
        ...
        const res = await axios.get(
          'http://localhost:2000/api/confirmed-parking/active',
          {
            params: { userId },
          }
        );
    ...
    const fetchHistory = async () => {
      try {
        const userId = localStorage.getItem('userId');
        const res = await axios.get(
          'http://localhost:2000/api/confirmed-parking',
          {
            params: { userId },
          }
        );
  ...
  const handleEndParking = async () => {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:2000/api/free-parking/${confirmedSpotId}`,
        {
          hasSpots: true,
          availableSpots: 1,
        }
      ...
      await axios.patch(
        `http://localhost:2000/api/confirmed-parking/${activeSession._id}/deactivate`
      );
